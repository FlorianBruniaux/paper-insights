...
**Statut : D1, D2 et D3 `APPROVED` le 2026-09-07; les 30 vérités D2 exactes sont `APPROVED` le 2026-09-08.** Le candidat `gate2-arxiv-metadata-v1` lie maintenant sa capture source, son catalogue, son index, son inventaire, ses requêtes et les résultats observés. Les 30 recherches ont une couverture complète à la révision 147: 25 placent au moins un papier attendu dans le top cinq, les misses sont les slots 13, 14, 24, 29 et 30, et aucun des six must-find n'échoue. Ces mesures ne sont pas des verdicts humains. Le Gate 2 reste `BLOCKED` à `0/30` revues.
...
**Décision humaine :** C2 et les cinq paramètres de sélection sont approuvés. Le manifeste source lie la sélection observée et ses limites. `prepared_queries.jsonl` est désormais l'autorité exacte approuvée avant exécution.
...
**État exécuté :** les 30 requêtes et ensembles attendus de `proposed_queries.jsonl`, SHA-256 `65d614cd22b1443f246f3e8545762b64b5be315c21436622c5c7d7db829524c7`, ont été approuvés explicitement le 2026-09-08. `prepared_queries.jsonl` en est une copie byte-for-byte portant la même empreinte. Le harnais a ensuite exécuté les 30 recherches et lié cette entrée dans `run_manifest.json`. Aucune requête ou attente n'a été modifiée après observation des résultats.
...
**Prochaine preuve attendue :** les 30 verdicts humains restent absents. Le résultat brut ne contient aucun miss dans les slots must-find, mais seul un reviewer humain peut enregistrer les 30 valeurs P0, son identité réelle et les timestamps de revue.
...
| `run_manifest.json` | `OBSERVED` | Schéma `search-relevance-run-v1`; SHA-256 `15208fe9a4fe371eeb65c7ba0f18deb639389a95608e7cfc3a999d2d54cdb7fc`; créé le `2026-09-08T06:13:32.539785+00:00`; 30 requêtes; statut de revue humaine `pending`. |
| SHA-256 de `prepared_queries.jsonl` et `observed_results.jsonl` | `OBSERVED` | `65d614cd22b1443f246f3e8545762b64b5be315c21436622c5c7d7db829524c7`; `e6f9afdb10840d44de7bb06fa9b50e8d199c13da5085175a8115455e8c4c950a`. Le manifeste lie ces entrées à la révision catalogue et à l'index observés. |
| `review_records_pre_review_sha256` | `OBSERVED` | `2aabad6573deb7b905a9e40cbed32aba15644ff157c8389c03b486b6e4683455`; les 30 lignes ont leurs champs `p0_relevance_failure`, `reviewer_id` et `reviewed_at` à `null`. |
| `review_form_sha256` | `OBSERVED` | `f303b7316373892416d8a4413dfd15a07e6c0615908061156ecce961d3b9805e`; formulaire généré avec attentes et résultats observés, sans verdict prérempli. |
...
Le répertoire observé contient les manifestes source, inventaire et run, l'inventaire, les requêtes approuvées, les résultats observés, les records de revue et le formulaire. Le harnais a enregistré 30 couvertures `complete`, 30 couples de révisions alignées, 25 correspondances top cinq et cinq misses. Les trois champs humains restent à `null` sur les 30 records. `validate-review` retourne donc `BLOCKED: 0/30 human reviews are complete` avec le code 2.
...
2. Réaliser les 30 revues humaines sans modifier requêtes, attentes ou résultats observés.
3. Produire `final_review_records.jsonl`, calculer son SHA-256 et exécuter `validate-review` contre le manifeste lié.
